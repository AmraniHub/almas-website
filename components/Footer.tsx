    ...
    <footer className="bg-[#0D0D1A] border-t border-[#C9A84C]/20 text-white/60 text-sm">
      <div className="max-w-7xl mx-auto px-6 py-14 grid grid-cols-1 md:grid-cols-4 gap-10">
        <div className="md:col-span-2">
          ...
          <a href={WA('Bonjour Almas, je souhaite passer une commande')}
            target="_blank"
        ...
        <div>
          ...
          <ul className="space-y-2">
            {([['/', 'Home'], ['/products', 'Products'], ['/wholesale', 'Wholesale'], ['/about', 'About Us'], ['/contact', 'Contact']] as [string, string][]).map(([href, label]) => (
        ...
        <div>
          ...
          <ul className="space-y-2">
            <li>📍 {SITE.addressEn}</li>
            <li>📞 {SITE.phone}</li>
            <li>✉️ {SITE.email}</li>
            <li className="pt-2"><span className="text-white/40 text-xs">Open: Mon – Sat, 8am – 6pm</span></li>
          </ul>
        </div>
      </div>
      <div className="border-t border-white/10 px-6 py-4 max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-2">
        <span>© {new Date().getFullYear()} Almas Spices & Herbs. All rights reserved.</span>
